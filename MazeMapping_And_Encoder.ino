...
void MazeMapping(int* MazeX, int* MazeY, char Direction, bool atIntersection, bool atRight, bool atLeft, bool atExit){
  switch (Direction){ 
    case 'N':
      *MazeY = *MazeY - 1; 
      break; 
  ...
  if(!atIntersection && atRight == true){  
      MazeMap[*MazeX][*MazeY][1] = 'F';   //No Node Forward   //Add Forward Detection
  ...
  if(!atIntersection && !atLeft && !atRight == true){
...
void loop() {
  ...
  //Stop if at an intersection MUST IMPLEMENT HANDLE INTERSECTION
    ...
    MazeMapping(&MazeX, &MazeY, Direction, atIntersection, atRight, atLeft, atExit); 
    mouse_right();

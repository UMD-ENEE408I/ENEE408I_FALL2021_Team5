            ...
            async with BleakClient(d.address) as client:
                ...
                while True:
                    ...
                    if (not(MazeX == prevMazeX) or not(MazeY == prevMazeY)):
                        Move_Count = 1
                        if(not(atIntersection == prevatIntersection) or not(atDeadEnd == prevatDeadEnd) or not(atRight == prevatRight) or not(atLeft == prevatLeft)):
                            Move_Count = 0
                            pos_print(MazeX, MazeY, Direction)
                            mapTheMaze()
                        elif (not(atExit == prevatExit) or not(travelledUnitLength == prevtravelledUnitLength)):    
                            Move_Count = 0
                            pos_print(MazeX, MazeY, Direction)
                            mapTheMaze()

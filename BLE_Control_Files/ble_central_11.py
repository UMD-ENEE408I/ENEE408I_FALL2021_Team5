            ...
            async with BleakClient(d.address) as client:
                ...
                while True:
                    ...
                    if((atIntersection == False) and (atDeadEnd == False) and (atRight == False) and (atLeft == False) and (atExit == False)): 
                        ...
                        mapTheMaze()
                    elif(not(atIntersection == prevatIntersection) or not(atDeadEnd == prevatDeadEnd) or not(atRight == prevatRight) or not(atLeft == prevatLeft) or not(atExit == prevatExit)):
                        Move_Count = 0
                        pos_print(MazeX, MazeY, Direction)
                        mapTheMaze()

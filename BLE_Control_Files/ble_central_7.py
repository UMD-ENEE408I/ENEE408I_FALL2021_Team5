            ...
            async with BleakClient(d.address) as client:
                ...
                while True:
                    ...
                    if (not(MazeX == prevMazeX) or not(MazeY == prevMazeY)):
                        Move_Count = 1
                        if(not(atIntersection == prevatIntersection) and not(atDeadEnd == prevatDeadEnd) and not(atRight == prevatRight) and not(atLeft == prevatLeft) and not(atExit == prevatExit)):
                            Move_Count = 0

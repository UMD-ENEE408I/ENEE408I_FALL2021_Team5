...
void moveCourseCorrect() {
  mouse_forward(FULL_SPEED_FORWARD + PIDvalue, (FULL_SPEED_FORWARD)- PIDvalue); //Added Plus 2 for Jacks Mouse
}
...
/*  for (int i = 0; i < 8; i++) {
  ...
  if (onWhiteLine == false) {
    if (ENABLE_MOVEMENT) {
      ...
      //Move Forward
      MADE_TURN_DISTANCE = 150; //Update this value to 150 becuase we have Traversed Extra Distance
      mouse_forward(40,40);
      delay(315);
      mouse_stop();
      delay(1000);
  ...
  //Stop if at an intersection MUST IMPLEMENT HANDLE INTERSECTION
   ...
   if (ENABLE_MOVEMENT) { 
    ...
    mouse_forward(40,40);
    delay(315);
    MADE_TURN_DISTANCE = 150; //Set distance of next Line Already traversed
    mouse_stop();
    ADCRead(); 
    //Check For End
    if ((isWhite(adc2_buf[0])) && (isWhite(adc1_buf[1])) && (isWhite(adc2_buf[1])) && (isWhite(adc1_buf[2])) && (isWhite(adc2_buf[2])) && (isWhite(adc1_buf[3])) && (isWhite(adc2_buf[3])) && (isWhite(adc1_buf[4])) && (isWhite(adc2_buf[4])) && (isWhite(adc1_buf[5])) && (isWhite(adc2_buf[5])) && (isWhite(adc1_buf[6]))) {
    ...
    if ((isWhite(adc2_buf[1])) || (isWhite(adc1_buf[2])) || (isWhite(adc2_buf[2])) || (isWhite(adc1_buf[3])) || (isWhite(adc2_buf[3])) || (isWhite(adc1_buf[4])) || (isWhite(adc2_buf[4])) || (isWhite(adc1_buf[5]))) { 
      IsForward = true; 
    }
    delay(1000);
    MazeMapping(&MazeX, &MazeY, Direction, atIntersection, atRight, atLeft, atExit, travelledUnitLength, IsForward, atDeadEnd); 
    ...
    for (int i = 0; i < 10; i++) {
     ...
     /* M1_forward(45);
      ...
      counterintcheck = counterintcheck + 1;
         ...
         ADCRead(); 
         //If still on White Line there is a Forward
         if ((isWhite(adc1_buf[2])) || (isWhite(adc2_buf[2])) || (isWhite(adc1_buf[3])) || (isWhite(adc2_buf[3])) || (isWhite(adc1_buf[4])) || (isWhite(adc2_buf[4]))) { 
            IsForward = true; 
         }
         delay(1000);
         MazeMapping(&MazeX, &MazeY, Direction, atIntersection, atRight, atLeft, atExit, travelledUnitLength, IsForward, atDeadEnd); 
 ...
 //On the line
    ...
    if (enc_set == false) {
      ...
      MADE_TURN_DISTANCE = 0; //Update this value to zero becuase we have not turned recently
      MazeMapping(&MazeX, &MazeY, Direction, atIntersection, atRight, atLeft, atExit, travelledUnitLength, IsForward, atDeadEnd);
      Serial.println();
      mouse_stop();
      delay(1000);

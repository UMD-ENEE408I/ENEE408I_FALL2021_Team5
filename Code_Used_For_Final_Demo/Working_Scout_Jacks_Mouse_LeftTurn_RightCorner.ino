...
const int LENGTH_AFTER_INT = 150;
...
int MazeX = 44;  //Initial X Position for the MazeMap is the Center
...
void Navigate_Left_Priority(int MazeX, int MazeY, bool atIntersection, bool atRight, bool atLeft, bool IsForward) {
  ...
  if (IsForward == false) {
    ...
    if (atIntersection == true) {
      if ((Check_Visited_Left(&MazeX, &MazeY)) == false) {
        mouse_left(); //Go Right if the mouse has not visited the node to the right
      }
      else if ((Check_Visited_Left(&MazeX, &MazeY)) == true) {
    ...
    //At Left Turn
      ...
      mouse_left(); //Go Right (Only Option)
    }

    //At Right Turn
    else if (!atIntersection && atRight == true) {
      mouse_right(); //Go Left (Only Option)
    }
  }
  ...
  //There is a Forward Node Detected
    ...
    if (atIntersection == true) {
      if ((Check_Visited_Left(&MazeX, &MazeY)) == false) {
        ...
        if ((Check_Visited_Forward(&MazeX, &MazeY)) == false) { //If right node has already been visited but forward node has not go forward
          ...
          if ((Check_Visited_Right(&MazeX, &MazeY)) == false) {
            mouse_right(); //Go Right if both the left Node and Forward Node have already been visited
          }
          else {
            mouse_left(); //If all Nodes have been visited already go left
          }
        }
      }
    }
    ...
    //At Left Turn With Forward Node
      ...
      if ((Check_Visited_Left(&MazeX, &MazeY)) == false) {
        ...
        if ((Check_Visited_Forward(&MazeX, &MazeY)) == false) {
          //Go Forward if forward node is unvisited and left node has been visited
        }
        else {
          mouse_left(); //Go right if both nodes have been visited
        }
      }
    }
    ...
    //At Right Turn with Forward Node
      ...
      if ((Check_Visited_Forward(&MazeX, &MazeY)) == false) {
        ...
        if ((Check_Visited_Right(&MazeX, &MazeY)) == false) {
          mouse_right(); //Go Right if right node is unvisited and forward node has been visited
        }
        else {
...
void loop() {
  ...
  while (central.connected()) {
    ...
    //______________________END BLUETOOTH_____________________
    ...
    if (atExit == true) {
      ...
      //Stop if at an intersection MUST IMPLEMENT HANDLE INTERSECTION
        ...
        if (ENABLE_MOVEMENT) {
          ...
          MazeMapping(&MazeX, &MazeY, Direction, atIntersection, atRight, atLeft, atExit, travelledUnitLength, IsForward, atDeadEnd);
          //Do Navigation
          Navigate_Left_Priority(MazeX, MazeY, atIntersection, atRight, atLeft, IsForward);
          IsForward = false;

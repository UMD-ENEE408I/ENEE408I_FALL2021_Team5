...
void executeCommand(int com) {
  ...
  if (com == 1) {
    commandCharacteristic.writeValue((byte) 10);
    moveForwardUntilIntersection();
    Serial.println("executed command: 1");
    commandCharacteristic.writeValue((byte) 20);
  }
  //command = turn left and move forward
    ...
    mouse_left();
    moveForwardUntilIntersection();
    Serial.println("executed command: 2");
    commandCharacteristic.writeValue((byte) 20);
  }
  //command = turn right and move forward
    ...
    mouse_right();
    moveForwardUntilIntersection();
    Serial.println("executed command: 3");
    commandCharacteristic.writeValue((byte) 20);
  }
  //command = begin
  /*else if (com == 5) {
...
void loop() {
  ...
  while (central.connected()) {
    ...
    byte received = 0;
    commandCharacteristic.readValue(received);
    if (((int) received) != 0) {
      ...
      executeCommand(commandCharacteristic.value());

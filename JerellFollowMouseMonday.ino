...
bool enc_set = false;
const int FULL_UNIT_LENGTH = 10000;
const int LENGTH_AFTER_INT = 200;
int MAZE_UNIT_LENGTH = FULL_UNIT_LENGTH; //was 1080
const int ENCODER_ERROR_BOUND = 100;
///////////////////////////////////////////////
...
int commands[] = {0, 1, 0, 3, 0, 2, 0, 3, 0, 3, 0, 2, 0, 2, 0, 3, 0, 2};
...
//Handle Command
void follow_command(int commands[50]) {
...
void loop() {
  ...
  //At Right Turn
  if (((isWhite(adc2_buf[0])) && (isWhite(adc1_buf[1])) && (isWhite(adc2_buf[1]))) && !(((isWhite(adc2_buf[4])) || (isWhite(adc1_buf[5])) || (isWhite(adc2_buf[5])) || (isWhite(adc1_buf[6]))))) {
    atRight = true;
  ...
  //Stop if at an intersection MUST IMPLEMENT HANDLE INTERSECTION
    ...
    if (ENABLE_MOVEMENT) {
      ...
      MazeMapping(&MazeX, &MazeY, Direction, atIntersection, atRight, atLeft, atExit, travelledUnitLength, IsForward, atDeadEnd);
      //Do Navigation
      if (commands[counterforcommand] == 1) {
        mouse_forward(40, 40);
        delay(10);
      } else if (commands[counterforcommand] == 2) {
        mouse_left();
      } else if (commands[counterforcommand] == 3) {
        mouse_right();
      }
      mouse_stop();

...
/* FEATURES INCLUDED:
  ...
  - Maze Mapping
  - Right Hand Rule Navigation with Node Memory
  - Bluetooth connection to Jetson
  - Sends whereabouts via Bluetooth
  - Has debugging #define statements
*/

#define SCOUT_MODE true //set this to true for scout mode. otherwise mouse is follow mode
const int SCOUT_NUM = 1; //set this to designated mouse number

#define DEBUG false //set this to true to only begin when connected to Serial monitor
#define MAP_DEBUG true //set this to true to Serial.print map data
...
/*
   The mouse will execute its hard written code only if told to begin by Jetson.
   When told to begin, isReady will be set to true.
*/
bool isReady = false;

bool ble_init; //state for whether or not ble is connected to Jetson
//__________________________________END MOUSE MODE_______________________________

//_____________________________BLUETOOTH_________________________________________
...
//_________SCOUT 1___________
const char* scoutServiceUuid = "19d10000-e8f2-537e-4f6c-d104768a1214";
const char* scoutServiceCharacteristicUuid = "19d10001-e8f2-537e-4f6c-d104768a1214";
...
//___________FOLLOWER___________________
const char* followServiceUuid = "29d10000-e8f2-537e-4f6c-d104768a1214";
const char* commandCharacteristicUuid = "19d10002-e8f2-537e-4f6c-d104768a1214";

BLEService scoutService(scoutServiceUuid); // BLE Service
BLEService followService(followServiceUuid);
...
bool enc_set = false;
const int FULL_UNIT_LENGTH = 300;
const int LENGTH_AFTER_INT = 175;
int MAZE_UNIT_LENGTH = FULL_UNIT_LENGTH; //was 1080
const int ENCODER_ERROR_BOUND = 110;
///////////////////////////////////////////////
...
const int Maze_Rows = 50;
const int Maze_Columns = 50;
char MazeMap[Maze_Rows][Maze_Columns][8];
/*fieds per node are:
...
//Forward
const unsigned int FULL_SPEED_FORWARD = 32;
...
const int BLACK = 680; //Changed to 675 from 680 for Testing.
...
void moveCourseCorrect() {
  mouse_forward((FULL_SPEED_FORWARD+3.5) + PIDvalue, (FULL_SPEED_FORWARD) - PIDvalue);
}
...
void setup() {
  ...
  //_________________BLUETOOTH______________________________
  ...
  if (SCOUT_MODE) {
    if (SCOUT_NUM == 1) {
      BLE.setLocalName("SCOUT1");
    }
    else {
      BLE.setLocalName("SCOUT2");
    }
  }
  else {
  ...
  //use follow service
    ...
    BLE.setAdvertisedService(followService);
  
    // add the characteristic to the service
    followService.addCharacteristic(commandCharacteristic);
  
    //add service
    BLE.addService(followService);
  
    //initialize command to no op
    commandCharacteristic.writeValue(0b00000000);
  
    //start advertising
    ble_init = false;
    BLE.advertise();
    delay(100);
    Serial.println("Follow Peripheral Service Started");
  }

  //________________________END BLUETOOTH__________________________
  ...
  int m;
  int l;
  for (m = 0; m < Maze_Rows; m++) {
    for (l = 0; l < Maze_Columns; l++) {
      MazeMap[m][l][0] = 'X';
      MazeMap[m][l][1] = 'X';
      MazeMap[m][l][2] = 'X';
      MazeMap[m][l][3] = 'X';
  ...
  MazeMap[MazeX][MazeY][0] = 'S';   //Mark the Node for the Start position location
  MazeMap[MazeX][MazeY][6] = 'V';   //Mark the Start Position to Visited
}

/*
   Sends the x and y position to Jetson.
   X and Y are their own characteristic, and X includes the mouse current direction in its left-most 2 bits.
   00 = N
   01 = S
   10 = E
   11 = W
...
void sendPosition() {
  ...
  xposCharacteristic.writeValue((byte) MazeX);

  yposCharacteristic.writeValue((byte) MazeY);

  if (Direction == 'N') {
    directionCharacteristic.writeValue(0x00);
  }
  else if (Direction == 'S') {
    directionCharacteristic.writeValue(0x01);
  }
  else if (Direction == 'E') {
    directionCharacteristic.writeValue(0x10);
  }
  else {
...
void MazeMapping(int* MazeX, int* MazeY, char Direction, bool atIntersection, bool atRight, bool atLeft, bool atExit, bool travelledUnitLength, bool IsForward, bool atDeadEnd) {
  ...
  */
  ...
  if (atExit == true) {
    MazeMap[*MazeX][*MazeY][0] = 'E';         //Set Exit to E
...
/*
 * takes in the raw command byte transmitted 
 * returns the designated mouse number the command
 * is intended for
 */
int getCommandMouseNumber(byte com) {
  if (com & 0b01000000 == 0b01000000) {
    return 1;
  }
  else if (com & 0b10000000 == 0b10000000) {
    return 2;
  }
  else if (com & 0b11000000 == 0b11000000) {
    return 3;
  }
  else {
    return -1; //error
...
/*
 * Takes in the raw command byte transmitted
 * returns the designated instruction integer
 */
int getCommand(byte com) {
  int toReturn = (com | 0b00111111);
  return (int) toReturn;
}

/*
 * decodes and executes command int received
 */
void executeCommand(int com) {
  //command = stop
  if (com == 0) {
    mouse_stop();
    commandCharacteristic.writeValue(0b00000000);
  ...
  //command = turn right and move forward
  else if (com == 3) {
    //move forward a little
    for(int i = 0; i<27; i++){
      mouse_forward(44,40);
...
/*
 * Mouse goes forward while on the line until an intersection or turn
 * is reached and then stops.
 * 
 * Does not account for errors in following the line
 */
void moveForwardUntilIntersection() {
  bool isExecuting = true;
  while (isExecuting) {
    ADCReadUpdate();
    if (onWhiteLine && !atIntersection && !atRight && !atLeft) {
      if (ENABLE_MOVEMENT) {    
...
/*
 * Reads ADC values and updates whereabouts booleans
 */
void ADCReadUpdate() {
  for (int i = 0; i < 8; i++) {
    adc1_buf[i] = adc1.readADC(i);
  ...
  //Detects when the mouse is completely off of the line
          ...
          //At Right Turn
          if (((isWhite(adc2_buf[0])) && (isWhite(adc1_buf[1])) && (isWhite(adc2_buf[1])))&& !(((isWhite(adc2_buf[4])) || (isWhite(adc1_buf[5])) || (isWhite(adc2_buf[5])) || (isWhite(adc1_buf[6]))))){ // && (isWhite(adc1_buf[2])) && (isWhite(adc2_buf[2])) && (isWhite(adc1_buf[3]))) && !(((isWhite(adc2_buf[4])) || (isWhite(adc1_buf[5])) || (isWhite(adc2_buf[5])) || (isWhite(adc1_buf[6]))))) {
            atRight = true;
          }
          else {
            atRight = false;
          ...
          //At Left Turn
          if (((isWhite(adc2_buf[4])) && (isWhite(adc1_buf[5])) && (isWhite(adc2_buf[5])) && (isWhite(adc1_buf[6]))) && !((isWhite(adc1_buf[0])) || (isWhite(adc2_buf[0])) || (isWhite(adc1_buf[1])) || (isWhite(adc2_buf[1])))) {
            atLeft = true;
          }
          else {
            atLeft = false;
...
void loop() {
  ...
  //while (!central.connected()) //stay here until connected to Jetson

  // if a central is connected to peripheral:
  if (central && !ble_init) {
    Serial.print("Connected to central: ");
    //print central MAC address
    Serial.println(central.address());
    ble_init = true;

    //while central still connected to peripheral
    /*while (central.connected()) {
      byte toSend = B00100001; //decimal 2
      Serial.print(toSend);
      statusCharacteristic.writeValue(toSend);
      Serial.print(toSend);
      }*/
  ...
  //Report whereabouts to Jetson
  while (central.connected()) {
//________________________________________________________RECEIVE COMMANDS__________________________________________
    //if the command characteristic has been updated by Jetson
    if (commandCharacteristic.valueUpdated()) {
      //if the command posted in the characteristic is intended for THIS mouse
      //NOTE: a mouse number of 0 is default for a no op command of 0b00000000
      }
      if (getCommandMouseNumber(commandCharacteristic.value()) == SCOUT_NUM) {
        //execute the command received
        executeCommand(getCommand(commandCharacteristic.value()));
      }
    
//________________________________________________________END RECEIVE COMMANDS_______________________________________________________
    
//_________SCOUT MODE CODE____________________________________________________________SCOUT MODE CODE_________________________________________//
          if (SCOUT_MODE) {//if (SCOUT_MODE && isReady) {
          //reportStatus();
            ...
            //Stop if at an intersection
              ...
              if (ENABLE_MOVEMENT) {
                ...
                for(int i = 0; i<27; i++){
                  ...
                  if ((isWhite(adc2_buf[0])) && (isWhite(adc1_buf[1])) && (isWhite(adc2_buf[1])) && (isWhite(adc1_buf[2])) && (isWhite(adc2_buf[2])) && (isWhite(adc1_buf[3])) && (isWhite(adc2_buf[3])) && (isWhite(adc1_buf[4])) && (isWhite(adc2_buf[4])) && (isWhite(adc1_buf[5])) && (isWhite(adc2_buf[5])) && (isWhite(adc1_buf[6]))) {            atIntersection = true; 
                ...
                MazeMapping(&MazeX, &MazeY, Direction, atIntersection, atRight, atLeft, atExit, travelledUnitLength, IsForward, atDeadEnd);
                //Do Navigation
                Navigate_Right_Priority(MazeX, MazeY, atIntersection, atRight, atLeft, IsForward);
                IsForward = false;
...
//_________SCOUT MODE CODE____________________________________________________________END SCOUT MODE CODE_________________________________________//
//_________FOLLOW MODE CODE___________________________________________________________FOLLOW MODE CODE___________________________________________________//
        if (!SCOUT_MODE) {//if (!SCOUT_MODE && !isReady) {
          /*
           * This area is where follow mode goes when NOT READY
           * 
           * The command receiving area in the beginning may already handle commands properly
           */
           mouse_stop();
        }
   }
